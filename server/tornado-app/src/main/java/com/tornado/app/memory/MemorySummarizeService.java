...
import com.tornado.infrastructure.memory.MemoryConfig;
...
/**
 * 长期记忆沉淀：会话归档（PATCH archived=1）时异步调便宜模型抽取 ≤5 条 {category,content}，
 * 简单去重后 upsert 入 long_term_memory（经 app.LongTermMemoryService）。每日兜底扫描归档超 7 天未沉淀的会话。
 */
@Slf4j
@Service
...
public class MemorySummarizeService {
    ...
    public void summarizeSession(Long sessionId) {
        try {
            ...
            List<ChatMessage> messages = messageRepository.listBySessionRolesAsc(sessionId, List.of("user", "assistant"));
            if (messages.isEmpty()) {
                return;
            }
